...
<header id="site-header" class="fixed top-0 inset-x-0 z-50 transition-all duration-300 bg-transparent">
  <nav class="max-w-7xl mx-auto px-5 sm:px-8 h-20 flex items-center justify-between">
    <a href="index.php" class="flex items-center gap-3 shrink-0 group">
      <?php if (!empty($p['logo'])): ?>
        <img src="<?= e(photo_url($p['logo'])) ?>" alt="<?= e($p['nom_restaurant']) ?>" class="h-11 w-11 rounded-xl object-cover ring-2 ring-brand-600/40 group-hover:ring-brand-600 transition">
      <?php else: ?>
        <span class="h-11 w-11 rounded-xl bg-ink text-brand-500 grid place-items-center font-display text-xl ring-2 ring-brand-600/40">
          <?= e(mb_substr($p['nom_restaurant'], 0, 1)) ?>
        </span>
      <?php endif; ?>
      <span class="font-display text-xl sm:text-2xl tracking-wide nav-brand-text">
        <?= e($p['nom_restaurant']) ?>
      </span>
    </a>

    <ul class="hidden lg:flex items-center gap-8 font-semibold text-sm nav-links">
    ...
    <a href="<?= e(whatsapp_link('Bonjour, je souhaite passer une commande.')) ?>" target="_blank" rel="noopener"
       class="hidden lg:inline-flex items-center gap-2 bg-brand-600 hover:bg-brand-700 text-white font-bold px-5 py-2.5 rounded-full text-sm shadow-glow transition hover:-translate-y-0.5">
      <i class="fa-brands fa-whatsapp text-base"></i> Commander
    </a>

    <button id="btn-burger" class="lg:hidden p-2 nav-burger" aria-label="Ouvrir le menu" aria-expanded="false">
  ...
  <div id="mobile-menu" class="lg:hidden fixed inset-0 top-20 bg-ink/98 backdrop-blur-xl translate-x-full transition-transform duration-300 ease-out overflow-y-auto">
    <ul class="flex flex-col gap-1 px-6 py-8 text-lg font-semibold text-white">
    ...
    <div class="px-6 pb-10">
      <a href="<?= e(whatsapp_link('Bonjour, je souhaite passer une commande.')) ?>" target="_blank" rel="noopener"
         class="flex items-center justify-center gap-2 bg-brand-600 text-white font-bold px-6 py-4 rounded-full">
